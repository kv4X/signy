...
export const SigninScreen = ({ navigation }) => {
  ...
  /* EMAIL */
  const [email, setEmail] = React.useState('almir.kvakic@gmail.com');
  ...
  async function loginHandler(){
    try{
      ...
      //let email = (response.user.email !== null);
      //if(email) navigation.navigate("Home");
    }catch(error){
      ...
      }else if(error.message == "USER_NOT_FOUND" || error.message == "INVALID_CRADENTIALS"){
        Toast.show({
          type: 'error',
          text1: 'Login error',
